...
# output
        ...
        self.mix_conv = nn.Conv1d(self.m*self.steps,self.m*self.steps,kernel_size=1,stride=1)
        self.bn = nn.BatchNorm1d(self.m*self.steps)
        self.active = nn.ELU()
    ...
    def forward(self, x):
        ...
        ts = ts.view(-1,preds,pixes//preds)
        r = r.permute(1,0,2).contiguous()
        r = r.view(-1,preds,pixes//preds)
        mix = torch.cat([ts,r],dim=2)
        # print(mix.shape)
        mix = self.mix_conv(mix)
        mix = self.bn(mix)
        # mix = self.active(mix)
        mix = torch.unsqueeze(mix,3)
        # print(mix.shape)
        out,_ = self.atten(mix,mix,mix)
        # print(out.shape)
        out = self.last_maxpool(out)
        out = torch.squeeze(out,1)
        out = out.view(-1,self.steps,self.m)
